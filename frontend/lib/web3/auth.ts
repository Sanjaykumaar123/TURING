...
export function mapWalletAuthError(error: unknown): string {
  ...
  if (message.includes("unsupported chain id")) {
    const configuredNetwork = process.env.NEXT_PUBLIC_NETWORK_NAME?.trim() || "jaringan yang dikonfigurasi";
    return `Network tidak sesuai. Pindah ke ${configuredNetwork} lalu coba lagi.`;
  }
