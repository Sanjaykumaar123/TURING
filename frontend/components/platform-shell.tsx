...
export function PlatformShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    setWalletSession(getWalletSessionFromDocumentCookie());
    setMounted(true);
  }, []);

  const walletAddress = mounted ? walletSession?.address : undefined;
  const walletRole = mounted
    ? (walletSession?.role ? walletSession.role.toUpperCase() : "No active wallet")
    : "No active wallet";
          ...
          >
            ...
            <div>
              ...
              <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-muted sm:text-[11px] sm:tracking-[0.24em]">
                Confidential Asset Intelligence
              </p>
            </div>
          </Link>
          <button
        ...
        <header className="sticky top-0 z-30 border-b border-border bg-white/95 backdrop-blur-xl">
          <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-2 px-3 py-3.5 sm:gap-4 sm:px-6 sm:py-4 lg:px-8">
            <div className="flex min-w-0 items-center gap-2 sm:gap-3">
              ...
              <p className="truncate text-sm font-semibold text-foreground sm:hidden">AEGIS AI</p>
