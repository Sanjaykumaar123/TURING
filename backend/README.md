# AEGIS AI Backend (Rust + Axum + PostgreSQL)

Modular backend for AEGIS AI with PostgreSQL-backed repositories and role-based API guards.
...
- `DATABASE_URL` (default: `postgres://postgres:postgres@127.0.0.1:5432/aegisai_backend`)
...
```bash
docker build -t aegisai-backend:latest .
```
...
docker run --rm -p 8080:8080 \
  -e DATABASE_URL=postgres://postgres:postgres@host.docker.internal:5432/aegisai_backend \
  -e BACKEND_PORT=8080 \
  aegisai-backend:latest
```
